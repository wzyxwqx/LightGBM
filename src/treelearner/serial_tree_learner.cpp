...
Tree* SerialTreeLearner::FitThreshold(const std::vector<int>& leaf_pred, const Tree* old_tree, const score_t* gradients, const score_t* hessians) {
  ...
  std::vector<data_size_t> data_indices;
  //OMP_INIT_EX();
  //#pragma omp parallel for schedule(static)
  for (int iter = 0; iter < tree->num_leaves() - 1; iter++) {
    //OMP_LOOP_EX_BEGIN();
    if (!node_is_numparent[iter]) continue;
    data_indices.resize(0);
    for (int i  = 0; i < num_data_; ++i)
      if (leaf_parent[leaf_pred[i]] == iter && node_is_numparent[iter]) {
      data_indices.push_back(i);
    }
    int real_feature = tree->GetNodeFeature(iter);//this is used to get feature but may need to be changed later
    int feature = train_data_->InnerFeatureIndex(real_feature);
